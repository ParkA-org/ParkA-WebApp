...
export default function ParkingCard({ parking }: CardProps) {
  ...
  for (let i = parking?.rating; i < 5 && stars.length != 5; i++) {
    stars.push(<BsStar size="2em" style={{ marginRight: "0.5em" }} />);
  }
  return (
    <Container
      onClick={() =>
        router.push("/parking/detail/[id]", `/parking/detail/${parking.id}`)
      }
    >
      <CardImage
        src={
      ...
      <CardHeader>
        ...
        <div>{stars}</div>
        <h4>({parking?.rating ? parking?.rating : "4.20"})</h4>
      </CardHeader>
      ...
      <CardInformation>
        <h3>Dirección</h3>
        <p>
          {parking?.direction ? parking?.direction : "Calle Puerto Rico #175"}
        </p>
        <h3>Precio</h3>
        <p>
          {parking?.priceHours
            ? `${parking?.priceHours} $RD por hora`
            : "125.00 $RD por hora"}
        </p>
      </CardInformation>
      <CarStatus>
      ...
      <ButtonSection>
        <ActionButton>
          <MdScreenShare color="white" size="1.5em" />
          Compartir
        </ActionButton>
        <ActionButton
          onClick={(event) => {
            ...
            router.push(
              "/parking/checkout/[id]",
              `/parking/checkout/${parking.id}`
            );
          }}
        >
          <MdMap color="white" size="1.5em" />
          Ver en mapa
        </ActionButton>
        <ActionButton
          onClick={(event) => {
