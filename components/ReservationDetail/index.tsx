    ...
    <Container>
      <LeftSection>
        ...
        <ElementContainer>
          ...
          data.getAllUserVehicles.length > 0 ? (
            <>
              <StyledSelect
                onChange={(e) => {
                  const { target } = e;
                  const { value } = target;
                  const imgUrl = data.getAllUserVehicles.find(
                    (vehicle) => vehicle.id === value
                  ).mainPicture;
                  imgRef.current.src = imgUrl;
                  setCheckout({ ...checkout, vehicle: value });
                }}
              >
                {data &&
                  data.getAllUserVehicles.map((vehicle) => (
                    <option
                      key={vehicle.id}
                      value={vehicle.id}
                      data-image={vehicle.mainPicture}
                    >
                      {vehicle.model.name}
                    </option>
                  ))}
              ...
              <StyledImage
                src={data.getAllUserVehicles[0].mainPicture}
                ref={imgRef}
              />
            </>
          ) : (
      ...
      <RightSection>
        <ElementContainer>
          ...
          <HourPicker
            hourPrice={parseInt(parking.priceHours)}
            checkout={checkout}
            setCheckout={setCheckout}
            calendar={parking.calendar}
          />
        </ElementContainer>

        <SectionElement
          name="Costo total"
          value={checkout.total ? checkout.total.toString() : "0"}
        />
      </RightSection>
    </Container>
  );
