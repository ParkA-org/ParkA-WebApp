...
type Action =
  ...
  | {
      type: "update_range";
      payload: { id: string; day: string; value: RangeObject };
    }
  ...
  switch (action.type) {
    ...
    case "add_day":
      return {
        ...state,
        [action.payload.day]: [
          { id: getRandomInt(10000).toString(), start: 1200, finish: 1400 },
        ],
      };
    case "remove_day":
      let stateCopy = {
...
export default function ParkingForm({
  ...
  return (
    <Formik
      ...
      onSubmit={(values) => {
        let modifiedState = {};
        for (let [key, range] of Object.entries(
          state as Record<string, RangeObject[]>
        )) {
          ...
          <Container>
            ...
            <LeftSection>
              <Field
                ...
                value={
                  values.countParking ? values.countParking.toString() : ""
                }
              />
              <Field
                name="parkingName"
                ...
                >
                  {week.map((day, idx) => (
                    <CheckElement
                      ...
                      value={idx}
                      dispatch={dispatch}
                      presentationName={presentationalWeek[idx]}
                      checked={calendar[day].length > 0 ? true : false}
                    />
                  ))}
                </div>
                    ...
                    {featuresData.getAllFeatures.map((feature) => {
                      return (
                        <Field
                          ...
                          checked={
                            values.features.filter(
                              (curFeature) => curFeature.id === feature.id
                            ).length > 0
                          }
            ...
            <RightSection>
              <ImagePicker
                ...
                pictures={pictures}
              />
              <Button submit={true}>Editar parqueo</Button>
              <Button
                onClick={() => {
                  DeleteParking({
                    variables: {
