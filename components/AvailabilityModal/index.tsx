...
export default function AvailabilityModal({ parking }: ComponentProps) {
  const [GetParkingDisponibility, { data }] = useLazyQuery<ParkingAvailability>(
    GET_PARKING_DISPONIBILITY
  );
    ...
    for (let z = 0; z < parkingAvailableCalendar.length; z++) {
      ...
      if (new Date(calendar.date).getDate() === new Date(date).getDate() - 1) {
        for (let i = 0; i < calendar.schedules.length; i++) {
          if (
            hour < calendar.schedules[i].start / 100 ||
            hour > calendar.schedules[i].finish / 100
          ) {
            return false;
          }
        }
  ...
  const DisableCalendarHours = (
    ...
    for (let i = 0; i < calendar[week[dayOfWeek]].length; i++) {
      if (
        0 === calendar[week[dayOfWeek]][i].start / 100 &&
        24 === calendar[week[dayOfWeek]][i].finish / 100
      )
        return false;

      if (
        hour < calendar[week[dayOfWeek]][i].start / 100 ||
        hour > calendar[week[dayOfWeek]][i].finish / 100
      ) {
        return true;
      }
    }
      ...
      <TimeSection>
        ...
        <div>
          ...
          <DatePicker
            ...
            defaultValue={startDate}
            hideHours={(hour) =>
              DisableCalendarHours(hour, startDate, parking.calendar)
            }
