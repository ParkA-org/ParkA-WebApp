...
export default function MapViewer(): JSX.Element {
  ...
  const { loading, error, data, refetch } = useQuery<AllParkingData>(
    GET_PARKINGS,
    {
        ...
        <ControllersContainer>
          <ButtonsContainer>
            <ButtonSection>
              ...
              >
                <AiOutlineMenu color="#333" size="1.5rem" />
              </button>
              <Search panTo={panTo} />
            ...
            {markers.map((marker) => {
              return process.browser ? (
                <Marker
                  ...
                  icon={{
                    url: marker.isAvailable
                      ? `/icons/availableIcon.svg`
                      : `/icons/unavailableIcon.svg`,
  ...
  return (
    <SearchContainer>
      <Combobox onSelect={handleSelect} aria-label="custom places demo">
        ...
        />
        <ComboboxPopover
          style={{ zIndex: 15, marginTop: "0.35em", border: "none" }}
        >
          <ComboboxList>
            {status === "OK" &&
