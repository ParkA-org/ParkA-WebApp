...
export default function RegisterPersonalIdentificacion(): JSX.Element {
  ...
  if (birthPlaceError)
    return <pre>`Error ${JSON.stringify(birthPlaceError)}`</pre>;
              ...
              variables: {
                cuiInput: {
                  ...
                  placeOfBirth: birthPlacedata.getAllCountries.filter(
                    (country) => country.name == values.birthPlace
                  )[0].id,
                },
              },
            });
        ...
        >
          {({ values, errors, touched, handleBlur, handleChange }) => (
            <Form>
                    ...
                    >
                      {birthPlacedata.getAllCountries.map(
                        (birthPlace: BasicEntity) => (
                          <option value={birthPlace.name} key={birthPlace.name}>
                            {birthPlace.name}
                          </option>
                        )
                      )}
              ...
              <ActionSection>
                <NavigationLink href="/register" styled={true}>
                  Atrás
                </NavigationLink>
