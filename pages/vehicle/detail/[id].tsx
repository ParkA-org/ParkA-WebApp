...
export default function DetailVehicle() {
  ...
  const [GetVehicle, { data, loading, error }] = useLazyQuery(
    GET_VEHICLE_BY_ID
  );
  ...
  const { redirect, loading: userLoading, userStatus, user } = useContext(
    UserContext
  );
  ...
  useEffect(() => {
    if (router.query.id && reservationData) {
      setVehicleReservations(
        reservationData.getAllUserReservationsAsClient.filter(
          (reservation) => reservation.vehicle.id === router.query.id
        )
      );
    }
  }, [reservationData]);
        ...
        <Container>
          ...
          <section>
            ...
            <CircularButton color="#336F8B;">
              <p>{vehicleReservations.length}</p> Reservas Completadas
            </CircularButton>
            <CircularButton color="#B40909;">
              <p>0</p> Denuncias
            </CircularButton>
          </section>
          <section>
  ...
  return (
    <Layout pageTitle="Detalle de vehiculos">
      <h3>Cargando....</h3>
    </Layout>
  );
}
