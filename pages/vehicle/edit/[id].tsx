...
export default function EditVehicle(): JSX.Element {
  ...
  const {
    ...
    error: makersError,
    data: makersData,
  } = useQuery<MakersData>(GET_MAKES);
  const {
    loading: colorsLoading,
    error: colorsError,
    data: colorsData,
  } = useQuery<ColorsData>(GET_COLORS);
  const {
    loading: bodyStylesLoading,
    error: bodyStylesError,
    data: bodyStylesData,
  } = useQuery<BodyStylesData>(GET_BODY_STYLES);
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      makersData.getAllMakes.map((make) =>
  ...
  if (userStatus === true) {
    return (
      <Layout pageTitle="Formulario de Edición de Vehiculos">
        <div
          style={{ textAlign: "left", width: "80%", margin: "0 auto 2em auto" }}
        >
                ...
                <Container>
                  <LeftSideContainer>
                    ...
                    {colorsLoading ? (
                      ...
                      >
                        {colorsData.getAllColors.map((color: BasicEntity) => (
                          <option value={color.id} key={color.name}>
                            {color.name}
                          </option>
                        ))}
                      </SelectField>
  ...
  return (
    <Layout pageTitle="Editar vehiculo">
      <h3>Cargando....</h3>
    </Layout>
  );
}
